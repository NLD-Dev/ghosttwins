...
class CreditSubState extends MusicBeatState
{
	...
	{
		super.create();
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('tiny', 'shared'));
		bg.scale.x *= 1.55;
		bg.scale.y *= 1.55;
		bg.screenCenter();
		add(bg);
		...
		var txt:FlxText = new FlxText(0, 0, FlxG.width,
			"Thanks for playing this demo! \n \n Here are the people that worked on this mod! \n \n \n \n NLD  (DIRECTOR/PROGRAMMER) \n PUPPYRELP (DIRECTOR/ARTIST) \n BBPANZU (VOICE ACTOR) \n NOT SO GRACEFUL PENNY(VOICE ACTOR) \n VENGFUL_VENISON (VOICE ACTOR) \n LETHRIAL (CHARTER) \n PAPTRISHA (ANIMATOR/ARTIST) \n SAKARUCHIBI (ARTIST) \n TOHRU (CHARTER) \n MIKE GENO (COMPOSER) \n LIL_COCOA_BEES (MUSIC DEMO)\n LOUDD (PLAY TESTER) \n GRIMM (PLAY TESTER)",
			32);
