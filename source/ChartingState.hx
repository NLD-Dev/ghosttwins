...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var check_goop:FlxUICheckBox;
	var check_changeBPM:FlxUICheckBox;
	var stepperSectionBPM:FlxUINumericStepper;
	var check_notetype:FlxUINumericStepper;
	var check_altAnim:FlxUICheckBox;
	var check_goopAnim:FlxUICheckBox;
	...
	function addSectionUI():Void
	{
		...
		check_goopAnim = new FlxUICheckBox(10, 430, null, null, "Goop Arrows", 100);
		check_goopAnim.name = 'check_Goop';
		...
		//tab_group_section.add(check_goopAnim);
	...
	function addNoteUI():Void
	{
		...
		var stepperSusLengthLabell = new FlxText(74,80,'Note Type');

		check_goop = new FlxUICheckBox(10, 30, null, null, "Goop Note", 100);
		check_goop.name = 'check_goop';
		check_goop.checked = false;
		check_notetype = new FlxUINumericStepper(10, 80, 1, 0, 0, 999, 0);
		check_notetype.value = 0;
		check_notetype.name = 'check_notetype';

		var applyLength:FlxButton = new FlxButton(10, 100, 'Apply Data');

		tab_group_note.add(check_goop);
		tab_group_note.add(check_notetype);
		tab_group_note.add(stepperSusLength);
		tab_group_note.add(stepperSusLengthLabel);
		tab_group_note.add(stepperSusLengthLabell);
	...
	function updateSectionUI():Void
	{
		...
		check_goopAnim.checked = sec.goop;
	...
	function updateNoteUI():Void
	{
		if (curSelectedNote != null)
			...
			check_goop.checked = curSelectedNote[3];
			check_notetype.value = curSelectedNote[4];
	}
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var daWut = i[3];
			var daWhat:Int = i[4];
			var notee:Note = new Note(daStrumTime, daNoteInfo % 4);
			notee.visible = false;
			var note:Note = new Note(daStrumTime, daNoteInfo % 4, notee, false, daWut, daWhat);
			note.sustainLength = daSus;
			note.goop = daWut;
			note.noteType = daWhat;
			note.setGraphicSize(GRID_SIZE, GRID_SIZE);
			note.updateHitbox();
			note.goop = daNoteInfo > 7;
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if (i.strumTime == note.strumTime && i.noteData % 4 == note.noteData && i.goop == note.goop)
			{
	...
	private function newSection(lengthInSteps:Int = 16,mustHitSection:Bool = false,altAnim:Bool = true,goop:Bool = false):SwagSection
		{
			var sec:SwagSection = {
				lengthInSteps: lengthInSteps,
				bpm: _song.bpm,
	...
	function shiftNotes(measure:Int=0,step:Int=0,ms:Int = 0):Void
		{
			...
			for (daSection1 in 0..._song.notes.length)
				{
					newSong.push(newSection(16,_song.notes[daSection1].mustHitSection,_song.notes[daSection1].altAnim,_song.notes[daSection1].goop));
				}
			...
			for (daSection in 0...(_song.notes.length))
			{
				...
				newSong[aimtosetsection].goop = _song.notes[daSection].goop;
