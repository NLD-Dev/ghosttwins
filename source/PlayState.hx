...
class PlayState extends MusicBeatState
{
	...
	public static var stagee:String = '';
	...
	public static var dadtwo:Character;
	public static var dadthree:Character;
	...
	private var activee:Bool = false;
	public static var startDialogue = true;


	private var healthBarBG:FlxSprite;
	private var healthBar:FlxBar;
	private var songPositionBar:Float = 0;
	private var healthThing:FlxText;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'hat-tip' | 'spectral-spat' | 'scuffle' | 'illustrious':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ghostDialogue/' + SONG.song.toLowerCase()));
		}

		goop = new FlxSprite();
		goop.frames = Paths.getSparrowAtlas('phase3shitholestuff/shitgoop', 'ghosttwins');
		goop.animation.addByPrefix('goopone', 'goop1');
		goop.animation.addByPrefix('gooptwo', 'goop2');
		goop.cameras = [camHUD];
		goop.screenCenter();
		goop.setGraphicSize(Std.int(goop.width * 1.2));
		goop.x += 300;
		add(goop);
		...
		switch(SONG.song.toLowerCase())
		{
			case 'illustrious':
				stagee = 'operastage';
				SONG.player2 = 'twinsfour';
			case 'scuffle':
				health = 2;
				stagee = 'operastage_melt';
			default:
				stagee = 'operastage_old';
		}

		switch(stagee)
		{
			case 'operastage_melt':
				{
						defaultCamZoom = 0.9;
						curStage = 'operastage_melt';
						var stageBack:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('backGrounds/operastage_melt/stageback','ghosttwins'));
						stageBack.antialiasing = true;
						stageBack.scrollFactor.set(0.9, 0.9);
						stageBack.active = false;
						add(stageBack);
	
						var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('backGrounds/operastage_melt/stagefront','ghosttwins'));
			...
			case 'operastage':
				{
						defaultCamZoom = 0.9;
						curStage = 'operastage';
						var stageBack:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('backGrounds/operastage/stageback','ghosttwins'));
						stageBack.antialiasing = true;
						stageBack.scrollFactor.set(0.9, 0.9);
						stageBack.active = false;
						add(stageBack);
	
						var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('backGrounds/operastage/stagefront','ghosttwins'));
						stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						stageFront.updateHitbox();
						stageFront.antialiasing = true;
						stageFront.scrollFactor.set(0.9, 0.9);
			...
			default:
				{
						defaultCamZoom = 0.9;
						curStage = 'operastage_old';
						var stageBack:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('backGrounds/operastage_old/stageback','ghosttwins'));
						stageBack.antialiasing = true;
						stageBack.scrollFactor.set(0.9, 0.9);
						stageBack.active = false;
						add(stageBack);
	
						var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('backGrounds/operastage_old/stagefront','ghosttwins'));
						stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						stageFront.updateHitbox();
						stageFront.antialiasing = true;
						stageFront.scrollFactor.set(0.9, 0.9);
		...
		statix = new FlxSprite(-100).loadGraphic(Paths.image('glitch','ghosttwins'));
		statix.setGraphicSize(Std.int(statix.width * 2));
		statix.updateHitbox();
		statix.screenCenter();
		statix.antialiasing = true;
		add(statix);
		statix.alpha = 0;
		statix.cameras = [camHUD];
		...
		if(SONG.song.toLowerCase() == 'spectral-spat'){
			dadtwo = new Character(100, 100, 'twinstwogoop');
			dadtwo.alpha = 0.78;
		}
		if(SONG.song.toLowerCase() == 'illustrious'){
			dadthree = new Character(100, 100, 'twinsmarie');
			dadtwo = new Character(100, 100, 'twinstanner');
		}
		...
		// add the shit health thing
		healthThing = new FlxText(4,healthBarBG.y + 95,0,'100%', 16);
		healthThing.screenCenter(X);
		healthThing.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		healthThing.scrollFactor.set();
		add(healthThing);

		if (FlxG.save.data.downscroll)
			kadeEngineWatermark.y = FlxG.height * 0.9 + 45;
			healthThing.y = FlxG.height * 0.9 - 50;
	...
	function createGoop():Void{
		var Int = FlxG.random.int(1,2);
		if(Int == 1){
			goop.animation.play('goopone');
	...
	function createSpookyText(text:String, x:Float = -1111111111111, y:Float = -1111111111111):Void
	{
		statix.alpha = 0.5;
		if(!textrendered){
		spookyText = new FlxText(dad.x, dad.y);
		spookyText.setFormat("PWScratchy", 128, FlxColor.WHITE);
		spookyText.size = 250;
		spookyText.y -= 250;
		...
		FlxG.sound.play(Paths.sound('statics', 'ghosttwins'), 2);
	...
	function startSong():Void
	{
		...
		switch(curSong)
		{
			case 'Bopeebo' | 'Philly' | 'Blammed' | 'Cocoa' | 'Eggnog' | 'hat-tip': allowedToHeadbang = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if(curSong.toLowerCase() == 'scuffle')
		{		
		var sh_toy = -200 + -Math.sin(curStep / 4) * 120 * 0.45;
		var sh_tox = 0 -Math.cos(curStep / 4) * 120;
		...
		healthThing.text = Std.string(Math.floor(Std.parseFloat(Std.string((maxHealthProb) / 2)))) + '%';

		if (healthBar.percent < 20)
		{
			healthThing.setFormat(Paths.font("vcr.ttf"), 32, 0xFFFF6677, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
			iconP1.animation.curAnim.curFrame = 1;
		}
		else{
			if (healthBar.percent > 80)
			{
				healthThing.setFormat(Paths.font("vcr.ttf"), 32, 0xFFBCFFAD, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
				iconP1.animation.curAnim.curFrame = 2;
			}else
				healthThing.setFormat(Paths.font("vcr.ttf"), 32, 0xFFFFFFFF, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						{
							case 2:
								var evilTrail = new FlxTrail(dad, null, 100, 0, 0.3, 0.069);
								if(SONG.song.toLowerCase() == 'scuffle'){
									if(FlxG.save.data.distractions){
										add(evilTrail);
									}
								...
								if(SONG.song.toLowerCase() == 'spectral-spat'){
								dadtwo.playAnim('singUP' + altAnim, true);
								}
								if(SONG.song.toLowerCase() == 'illustrious'){
									if(daNote.noteType == 0){
										dadtwo.playAnim('singUP' + altAnim, true);
										dadthree.playAnim('singUP' + altAnim, true);
									}
							...
							case 3:
								...
								if(SONG.song.toLowerCase() == 'spectral-spat'){
								dadtwo.playAnim('singRIGHT' + altAnim, true);
								}

								if(SONG.song.toLowerCase() == 'illustrious'){
									if(daNote.noteType == 0){
										dadtwo.playAnim('singRIGHT' + altAnim, true);
										dadthree.playAnim('singRIGHT' + altAnim, true);
									}
									if(daNote.noteType == 2){
										dadtwo.playAnim('singRIGHT' + altAnim, true);
									}
							...
							case 1:
								...
								if(SONG.song.toLowerCase() == 'spectral-spat'){
								dadtwo.playAnim('singDOWN' + altAnim, true);
								}

								if(SONG.song.toLowerCase() == 'illustrious'){
									if(daNote.noteType == 0){
										dadtwo.playAnim('singDOWN' + altAnim, true);
										dadthree.playAnim('singDOWN' + altAnim, true);
									}
									if(daNote.noteType == 2){
										dadtwo.playAnim('singDOWN' + altAnim, true);
									}
							...
							case 0:
								...
								if(SONG.song.toLowerCase() == 'spectral-spat'){
									dadtwo.playAnim('singLEFT' + altAnim, true);
								}

								if(SONG.song.toLowerCase() == 'illustrious'){
									if(daNote.noteType == 0){
										dadtwo.playAnim('singLEFT' + altAnim, true);
										dadthree.playAnim('singLEFT' + altAnim, true);
									}
									if(daNote.noteType == 2){
										dadtwo.playAnim('singLEFT' + altAnim, true);
									}
						...
						#if web
						FlxG.sound.music.fadeOut(0, 0);
						FlxG.switchState(new AnimationTimer());
						#end
						if(dad.curCharacter == 'twinsmerged')
						{
								if(daNote.isSustainNote)
										health -= 0.005;
								else
						...
						if(FlxG.save.data.distractions){  
						var fuck = FlxG.random.int(1, 20);
						if(fuck == 5)
						{
							if(dad.curCharacter == 'twinsmerged')
							{
							createSpookyText('HELP');
							}
						}
						if(dad.curCharacter == 'twinstwo')
						{
							camera.shake(0.005, 0.3);
									var winX = Lib.application.window.x;
									var winY = Lib.application.window.y;  
									Lib.application.window.move(winX, winY - 7);
									Lib.application.window.move(winX, winY + 7);
						...
						if(dad.curCharacter == 'twinsmerged')
						{
									...
									camera.shake(0.007, 0.3);
									var aaaaa = FlxG.random.int(1, 3);
									if(aaaaa == 1)
									{
									var winX = Lib.application.window.x;
									var winY = Lib.application.window.y;  
									...
									var aaaaaa = FlxG.random.int(1, 3);
									if(aaaaaa == 1)
									{
									var winX = Lib.application.window.x;
									var winY = Lib.application.window.y;  
									Lib.application.window.move(winX - 15, winY);
									Lib.application.window.move(winX + 15, winY);
									Lib.application.window.move(winX, winY);
									}else
									{
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					if (storyDifficulty == 3 || storyDifficulty == 2)
					{
						if(SONG.song.toLowerCase() == 'scuffle')
						{
							PlayState.SONG = Song.loadFromJson("illustrious" + difficulty, "illustrious");
						}else{
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			if (noteDiff > Conductor.safeZoneOffset * 0)
					daTiming = 'early';
			if (noteDiff < Conductor.safeZoneOffset * 0)
					daTiming = 'late';
			...
			var timing:FlxSprite = new FlxSprite().loadGraphic(Paths.image(daTiming));
			timing.screenCenter();
			timing.x = comboSpr.x + 100;
			timing.y = rating.y + 90;
			timing.acceleration.y = 600;
			timing.velocity.y -= 150;

			comboSpr.velocity.x += FlxG.random.int(1, 10);
			currentTimingShown.velocity.x += comboSpr.velocity.x;
			if(!FlxG.save.data.botplay) add(rating);
			if(!FlxG.save.data.botplay) add(timing);
	
			if (!curStage.startsWith('school'))
			{
				rating.setGraphicSize(Std.int(rating.width * 0.7));
				rating.antialiasing = true;
				timing.setGraphicSize(Std.int(timing.width * 0.5));
			...
			else
			{
				...
				timing.setGraphicSize(Std.int(timing.width * daPixelZoom * 0.5));
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				#end
				if (controls.LEFT_P){if(konami == 4 || konami == 6){konami += 1;}else{konami = 0;}}
				if (controls.DOWN_P){if(konami == 2 || konami == 3){konami += 1;}else{konami = 0;}}
				if (controls.UP_P){if(konami == 0 || konami == 1){konami += 1;}else{konami = 0;}}
