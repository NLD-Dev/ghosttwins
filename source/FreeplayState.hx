...
class FreeplayState extends MusicBeatState
{
		...
		if(songs[curSelected].songName == 'Scuffle')
		{
			if(bop == false)
			{
				if(FlxG.save.data.distractions){
					camera.angle = -1;
					tween = FlxTween.tween(camera, {angle: 1}, 2, { ease: FlxEase.cubeInOut, type: PINGPONG });
					bop = true;
				}
			}
		}else{
			...
			tween.cancel();
			camera.angle = 0;
			tween = FlxTween.tween(camera, {angle: 0}, 2, { ease: FlxEase.cubeInOut, type: PINGPONG });
	...
	function changeDiff(change:Int = 0)
	{
		...
		if(songs[curSelected].songName == 'Illustrious')
		{
		if (curDifficulty < 2)
			curDifficulty = 3;
		if (curDifficulty > 3)
	...
	function changeSelection(change:Int = 0)
	{
		...
		tween = FlxTween.tween(camera, {angle: 1.5}, 2, { ease: FlxEase.cubeInOut, type: PINGPONG });
